...
**Company lens (independent):** Rocket Lab (`rocket_lab`)  
**Innovation:** Launch Hold Receipt  
**Scaffold batch:** 2026-08-10T0924Z

## Mission

Implement a **real, testable** central mechanism that addresses the bottleneck below. Do **not** claim Rocket Lab affiliation, proprietary access, or production deployment.

### Bottleneck
Maintaining one reliable software lifecycle across vehicles, spacecraft, test, simulation, and operations.

### Brick wall
Versioning and validating mission-critical software as products and missions diversify.

### Mechanism to implement
Encode hold reasons, authority, and expiry into a signed hold receipt that blocks progression until cleared.

## Hard rules (fail closed)

1. **No affiliation theater** — never state or imply Rocket Lab employment, endorsement, or proprietary systems access.
...
File: `src/launch_hold_receipt.py`

- Expand `LaunchHoldReceipt` into a complete, self-contained implementation.
- Public API must stay stable enough that tests in `tests/test_launch_hold_receipt.py` can be upgraded (not gutted).
- Include at least:
...
### 3. Strengthen tests
Files: `tests/test_launch_hold_receipt.py`, `tests/test_adversarial.py`
...
- Cloud deploy, customer pilots, proprietary Rocket Lab APIs
